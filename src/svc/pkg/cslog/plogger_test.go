...
import (
	...
	"github.com/briand787b/ncx/src/svc/pkg/util"

	diff "github.com/yudai/gojsondiff"
	"github.com/yudai/gojsondiff/formatter"
)

func TestCSLoggerError(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name              string
		spanID            *string
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.spanID != nil {
				ctx = cslog.StoreSpanID(ctx, *tt.spanID)
			}

			if tt.TraceID != nil {
				ctx = cslog.StoreTraceID(ctx, *tt.TraceID)
			}

			slw := cslogtest.SpyLogWriter{}
			ug := cslogtest.MockUUIDGen{StringRetStrings: tt.uuidGenSet}
			cslog.NewCSLogger(&slw, &ug).Error(ctx, tt.msg, tt.args...)
